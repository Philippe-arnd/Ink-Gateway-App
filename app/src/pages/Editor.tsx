...
export function Editor() {
  ...
  async function sendChat(e: React.FormEvent) {
    e.preventDefault();
    if (!id || !chatInput.trim() || chatBusy) return;
    ...
    try {
      for await (const event of streamChat(id, message)) {
        if (event.type === "text") {
          setChat((c) => [...c, { kind: "assistant", text: event.data }]);
          ...
          setChat((c) => [...c, { kind: "tool_result", text: event.data.name }]);
        } else if (event.type === "error") {
          setChat((c) => [...c, { kind: "tool_error", text: event.data }]);
        }
      }
    } finally {
      setChatBusy(false);
    ...
    <div className="editor-layout">
      ...
      <aside className="chat-panel">
        ...
        <div className="chat-log">
          ...
          {chatBusy && (
            <div className="chat-line tool_result">
              <IconSparkles className="icon spin" />
        ...
        <form onSubmit={sendChat}>
          <textarea
            ...
            placeholder="Continue l'histoire, réécris ce passage, ajoute un personnage…"
            rows={3}
          />
